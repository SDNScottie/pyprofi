...
class cSubnetting:
    ...
    def __init__(self, z_msg):
        ...
        print("\n")
        print("!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        print ( "constructor: cSubnetting " + z_msg )
        print("!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        print("\n")

    def add_2_numbers(self, z_nr_1, z_nr_2 ):
        return z_nr_1 + z_nr_2

    def calculate_power(self):
        info = print( self.__class__.__name__ + " , " + self.calculate_power.__name__ + " ( should be )  " )
        print(info)
        pass


    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def network_addressing_schemes(self):
        print("network_addressing_schemes")

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def wildcard_mask(self):

        print("wildcard_mask : for firewalls")
        print("wildcard_mask : for access listes")
        print("wildcard_mask : for routing protocols")

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def ipvx_addressing(self):
        print("ipvx_addressing")

    def ipv4_addressing(self):

        print("ipv4_addressing")

    def ipv6_adressing(self):
        print("ipv6_addressing")


    def do_binary_bits(self):
        #https://stackoverflow.com/questions/47879904/the-network-mask-prefix-and-dotted-decimal-format-transformation-in-python
        
        print( bin((~((1<<(32-24))-1)) & ((1<<32)-1)) )


    def do_netmask_bits(self):
    ...
    def do_prefix_length(self, z_prefix):
        print("~~~~~~~(prefix length from : " + z_prefix + " )~~~~~~~")
        print( IPNetwork("0.0.0.0/"+z_prefix).prefixlen )
        #print( self.netmask_to_cidr(z_prefix) )


    def do_ipnetwork(self, zIP_slash_subnetmask):
        ip_subnet = zIP_slash_subnetmask.split("/")


        #print( zIP_slash_subnetmask )

        #https://docs.python.org/3/library/ipaddress.html

        res = ipaddress.ip_network( zIP_slash_subnetmask )

        print( "~~~~~~~~~~~~~~~~~~~~~~~~~~" )       
        print( res )
        print( "~~~~~~~~~~~~~~~~~~~~~~~~~~" )       

        print("~~~~~( dotted decimal for : /" +ip_subnet[1]+ " )~~~~~")
        print( self.do_bits_to_mask( int(ip_subnet[1]) ) )

        print("~~~~~( reverse pointer )~~~~~")
        print( res.reverse_pointer )

        #print(" binary representation of this address ")
        #print( res.packed )

        print("~~~~~( supernet of the subnet )~~~~~")
        print( res.supernet() )

        print("~~~~~( usable hosts )~~~~~")
        #print( res.hosts() )
        usable_hosts = list( res.hosts() )   # in order to use generators output, need it in form of list
        first_usable_host = usable_hosts[0]
        last_usable_host = usable_hosts[-1]
        print("first usable host = " + str(first_usable_host))
        print(" last usable host = " + str(last_usable_host))

        
        print("\n\n")


class cSubnetType: # ___CodeChallenge__Nr01:
    ...
    def check_if_class_A(self, List_ip_address, slash_mask):
        ...
        #== Tip: Here the goal is break compare the List members and determine if IP is class A
...
# ==
# ============================================================
def main_cSubnetting():
    tst = cSubnetting("hello here, Pyaes")
    #tst.do_ipnetwork("192.168.0.0/28")
    #tst.do_prefix_length("255.255.255.0")
    #tst.do_ipnetwork("98.174.166.123/25")    # not a valid ip address
    tst.do_ipnetwork("192.168.0.0/16")
    #tst.do_prefix_length("255.255.255.128")

    #print( tst.do_bits_to_mask(24) )

    #tst.calculate_power()

def main_cSubnetType():
    cSubnetType("192.168.0.0","16")
